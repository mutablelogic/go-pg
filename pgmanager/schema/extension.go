...
func (q ExtensionListRequest) Query() url.Values {
	...
	if q.Installed != nil {
		values.Set("installed", fmt.Sprint(types.Value(q.Installed)))
	}
	return values
}
...
func (r Extension) Header() []string {
	return []string{"Oid", "Name", "Database", "Owner", "Schema", "Default Version", "Installed Version", "Relocatable", "Requires", "Comment"}
}

func (r Extension) Width(col int) int {
	return 0
}

func (r Extension) Cell(col int) string {
	switch col {
	case 0:
		if r.Oid != nil {
			return fmt.Sprint(types.Value(r.Oid))
		}
		return ""
	case 1:
		return r.Name
	...
	case 6:
		return types.Value(r.InstalledVersion)
	case 7:
		return fmt.Sprint(types.Value(r.Relocatable))
	case 8:
		return strings.Join(r.Requires, ", ")
	case 9:
		return r.Comment
	default:
		return ""
...
func (e *Extension) Scan(row pg.Row) error {
	return row.Scan(&e.Oid, &e.Name, &e.Owner, &e.Schema, &e.DefaultVersion, &e.InstalledVersion, &e.Relocatable, &e.Requires, &e.Comment)
}
