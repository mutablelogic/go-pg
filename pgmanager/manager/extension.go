...
func (manager *Manager) ListExtensions(ctx context.Context, req schema.ExtensionListRequest) (_ *schema.ExtensionList, err error) {
	// Otel span
	ctx, endSpan := otel.StartSpan(manager.tracer, ctx, "ListExtensions",
	...
	if database := strings.TrimSpace(types.Value(req.Database)); database != "" {
		...
		for i := range result.Body {
			result.Body[i].Database = database
		}
	} else {
	...
	result.ExtensionListRequest = req
...
func (manager *Manager) GetExtension(ctx context.Context, name string) (_ *schema.Extension, err error) {
	// Otel span
	ctx, endSpan := otel.StartSpan(manager.tracer, ctx, "GetExtension",
...
func (manager *Manager) CreateExtension(ctx context.Context, meta schema.ExtensionMeta, cascade bool) (_ *schema.Extension, err error) {
	// Otel span
	ctx, endSpan := otel.StartSpan(manager.tracer, ctx, "CreateExtension",
		attribute.String("meta", types.Stringify(meta)),
		attribute.Bool("cascade", cascade),
	)
	defer func() { endSpan(err) }()
...
func (manager *Manager) UpdateExtension(ctx context.Context, name string, meta schema.ExtensionMeta) (_ *schema.Extension, err error) {
	// Otel span
	ctx, endSpan := otel.StartSpan(manager.tracer, ctx, "UpdateExtension",
		attribute.String("name", name),
		attribute.String("meta", types.Stringify(meta)),
	)
	defer func() { endSpan(err) }()
...
func (manager *Manager) DeleteExtension(ctx context.Context, database, name string, cascade bool) (err error) {
	// Otel span
	ctx, endSpan := otel.StartSpan(manager.tracer, ctx, "DeleteExtension",
